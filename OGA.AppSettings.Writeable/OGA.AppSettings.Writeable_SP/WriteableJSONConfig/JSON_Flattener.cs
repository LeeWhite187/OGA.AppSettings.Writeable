...
namespace JsonFlatten_Dict_StringString
{
    public static class JsonFlattenStringStringExtensions
    {
        /// <summary>
        /// Flattens a JObject to a Dictionary.<c>null</c>, <c>""</c>, <c>[]</c> and <c>{}</c> are preserved by default
        /// </summary>
        ...
        /// </summary>
        /// <param name="flattenedJsonKeyValues">Dictionary to unflatten</param>
        public static JObject Unflatten(this IDictionary<string, string> flattenedJsonKeyValues)
        {
            JContainer result = null;
            var setting = new JsonMergeSettings
            {
        ...
        private static IList<string> SplitPath(string path)
        {
            ...
            result = path.Split(new char[] { ':' }, StringSplitOptions.RemoveEmptyEntries).ToList();

            return result;

            // Old method did the following...
            //var result = new List<string>();
            //var reg = new Regex(@"(?!\.)([^. ^\[\]]+)|(?!\[)(\d+)(?=\])");
            //foreach (Match match in reg.Matches(path))
            //{
