...
{
    public class AssemblyBase_Tests
    {
        ...
        static protected int Get_AppPath_List(out Config_AppPaths_v2 apppaths)
        {
            apppaths = null;
            try
            {
                string text = "Attempting to locate config folder...";
                Console.WriteLine(text);
                Logging_Base.Logger_Ref?.Debug(text);

                // We need to lookup the config path from appsettings, before the configuration builder has started.
                // So, we will start a temporary one.

                // Create a configuration builder, that will load the config file from the exe folder...
                var config = new ConfigurationBuilder()
                // Set the builder to look in the exe folder...
                    .SetBasePath(AppDomain.CurrentDomain.BaseDirectory)
                // Have it load the app settings file...
                .AddJsonFile(Config_AppPaths_v2.CONSTANT_ConfigFile)
                // Tell it to build, so we can use the collected config data...
                .Build();

                // Retrieve the configuration section we need...
                var appsettingssection = config.GetSection(Config_AppPaths_v2.CONSTANT_SectionName);

                // Hydrate the app settings data to our POCO...
                Config_AppPaths_v2 appsettings = appsettingssection.Get<Config_AppPaths_v2>();

                text = "Config folder located.";
                Console.WriteLine(text);
                Logging_Base.Logger_Ref?.Debug(text);
            ...
            {
                Logging_Base.Logger_Ref?.Error(e,
                    "Program:Get_AppPath_List - " +
                    "Failed to load app paths.");
